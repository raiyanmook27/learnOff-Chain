...
async function main() {
  //lets get accounts balcance before transaction
  const balanceBeforeAcc1 = await linkContract.balanceOf(account1);
  const balanceBeforeAcc2 = await linkContract.balanceOf(account2);
  //we need to connect to the wallet
  const wallet = new ethers.Wallet(privateKey, provider);

  //we send link token to account2
  const tx = await linkContract
    .connect(wallet)
    .transfer(account2, ethers.parseEther("10"));

  const details = await tx.wait();

  console.log(details);
  //lets get accounts balcance after transaction
  const balanceAfterAcc1 = await linkContract.balanceOf(account1);
  const balanceAfterAcc2 = await linkContract.balanceOf(account2);
